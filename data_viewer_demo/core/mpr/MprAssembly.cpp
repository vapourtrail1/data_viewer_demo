...
void MprAssembly::detach()
{
    ...
    if (m_axialWidget) {
        m_axialWidget->setRenderWindow(nullptr);
    }
    if (m_coronalWidget) {
        m_coronalWidget->setRenderWindow(nullptr);
    }
    if (m_sagittalWidget) {
        m_sagittalWidget->setRenderWindow(nullptr);
    }
    if (m_volumeWidget) {
        m_volumeWidget->setRenderWindow(nullptr);
    }
...
void MprAssembly::setup3DScene(QVTKOpenGLNativeWidget* widget)
{
    ...
    m_cursorActor->SetCursor(m_state->cursor());
